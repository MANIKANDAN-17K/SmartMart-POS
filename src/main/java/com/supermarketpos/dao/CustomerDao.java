...
/**
 * Billing-relevant customer queries.
 * Merge these methods into your existing CustomerDao.
 */
public void insert(Connection conn, Customer customer) throws SQLException {
    ...
    try (PreparedStatement ps = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
        ...
        if (rows == 0) throw new SQLException("Insert customer failed – no rows affected.");
...
public class CustomerDao {
    ...
    public List<Customer> searchByName(Connection conn, String keyword) throws SQLException {
        String sql = "SELECT * FROM customers WHERE name LIKE ? LIMIT 10";
        List<Customer> result = new ArrayList<>();
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, "%" + keyword.trim() + "%");
            try (ResultSet rs = ps.executeQuery()) {
    ...
    public List<Customer> searchByPhone(Connection conn, String phone) throws SQLException {
        String sql = "SELECT * FROM customers WHERE phone LIKE ? LIMIT 10";
        List<Customer> result = new ArrayList<>();
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, "%" + phone.trim() + "%");
            try (ResultSet rs = ps.executeQuery()) {
